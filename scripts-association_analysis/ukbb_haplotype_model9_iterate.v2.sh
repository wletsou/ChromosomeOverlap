...
# wrapper script for doing forward-selection in UKBB on each haplotype in counts files produced by ukbb_haplotype_model9_sub.v2.sh

# bsub -P SJLIFE -J ukbb_haplotype_model9_iterate.v2.discovery -oo ukbb_haplotype_model9_iterate.v2.discovery.out -eo ukbb_haplotype_model9_iterate.v2.discovery.err -R "rusage[mem=256]" "sh ukbb_haplotype_model9_iterate.v2.sh ukbb_discovery.allele_counts.txt ukb.bca.pheno.txt \"\" \"\" 1e-5 ukbb_discovery.Significant_patterns.txt \"ukbb_discovery\""
...
PHENOTYPES=$2 # phenotypes file for getting age ("age.end"), principal components of ancestry ("pc1" to "pc10") and affected status ("diag") from each subject ("sid")
...
if [ -z $CUTOFF ]
then
  CUTOFF=1e-5 # LRT p-value cutoff; stop when all p exceed
fi
...
do
  ...
  echo bsub \-P SJLIFE \-J \"myJob[1-$N_JOBS]\" \-oo ukbb_haplotype_model9_fit.v2.${NAME/%/.}round_${k}.%I.out \-eo ukbb_haplotype_model9_fit.v2.${NAME/%/.}round_${k}.%I.err \-R \"rusage[mem=4000]\" \-R \"order[!ut]\" \-R \"select[ut \< 0.9]\" \-q standard \"sh ${HOME_DIR/%/\/}ukbb_haplotype_model9_fit.v2.sh \\\"${INCLUDED_HAPLOTYPES%.*}.included_haplotypes.txt\\\" \\\"$(eval "echo ${NAME/%/.}included_haplotype_{0..${x}}.new_allele_counts.job\\\$LSB_JOBINDEX.txt | tr \" \" \",\"")\\\" \\\"$PHENOTYPES\\\" \\\"$3\\\" \\\"$4\\\" job\$LSB_JOBINDEX ${DIRECTORY/%/\/}${NAME/%/.}Conditional_haplotype_effects.$(eval "for i in {$((${#group_array[@]}+1))..$((${#group_array[@]}+$n_new_haplotypes))}; do printf \"%s,\" h\$i; done | sed 's/,\$//g'").txt $DIRECTORY $HOME_DIR\"
  bsub -P SJLIFE -J "myJob[1-$N_JOBS]" -oo ukbb_haplotype_model9_fit.v2.${NAME/%/.}round_${k}.%I.out -eo ukbb_haplotype_model9_fit.v2.${NAME/%/.}round_${k}.%I.err -R "rusage[mem=4000]" -R "order[!ut]" -R "select[ut < 0.9]" -q standard "sh ${HOME_DIR/%/\/}ukbb_haplotype_model9_fit.v2.sh \"${INCLUDED_HAPLOTYPES%.*}.included_haplotypes.txt\" \"$(eval "echo ${NAME/%/.}included_haplotype_{0..${x}}.new_allele_counts.job\\\$LSB_JOBINDEX.txt | tr \" \" \",\"")\" \"$PHENOTYPES\" \"$3\" \"$4\" job\$LSB_JOBINDEX ${DIRECTORY/%/\/}${NAME/%/.}Conditional_haplotype_effects.$(eval "for i in {$((${#group_array[@]}+1))..$((${#group_array[@]}+$n_new_haplotypes))}; do printf \"%s,\" h\$i; done | sed 's/,\$//g'").txt $DIRECTORY $HOME_DIR" && printf "\n"
