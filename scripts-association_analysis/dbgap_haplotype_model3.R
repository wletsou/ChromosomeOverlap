...
if (exists("deviance0") && exists("deviance1") && exists("df0") && exists("df1")) {
  ...
  cat(sprintf("LRT p value = %0.6e on %d degree(s) of freedom with OR = %0.6e (%0.6e to %0.6e), frequency = %0.6e (%d) = %0.6e/%0.6e (%d/%d), p = %0.6e\n",p,abs(df1-df0),OR,OR_ll,OR_ul,eval(parse(text = sprintf("freq_combined$h%s",length(ranges)))),eval(parse(text = sprintf("counts_combined$h%s",length(ranges)))),eval(parse(text = sprintf("freq_separate$h%s[2]",length(ranges)))),eval(parse(text = sprintf("freq_separate$h%s[1]",length(ranges)))),eval(parse(text = sprintf("counts_separate$h%s[2]",length(ranges)))),eval(parse(text = sprintf("counts_separate$h%s[1]",length(ranges)))),eval(parse(text = sprintf("fisher.pvalue$h%s[1]",length(ranges))))))
} else {
