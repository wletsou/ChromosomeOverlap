...
do
  ...
  echo Rscript ${HOME_DIR/%/\/}ukbb_haplotype_model.v2.R -i ${INCLUDED_HAPLOTYPES} -t ${TEST_HAPLOTYPES} -p $PHENOTYPES -k $((i+1)) -g "$groups" -v 1 -n $((${#group_array[@]}-${#JOIN[@]}))
  if [ -z $OUTPUT ]
  then
    Rscript ${HOME_DIR/%/\/}ukbb_haplotype_model.v2.R -i ${INCLUDED_HAPLOTYPES} -t ${TEST_HAPLOTYPES} -p $PHENOTYPES -k $((i+1)) -g "$groups" -v 1 -n $((${#group_array[@]}-${#JOIN[@]}))
    printf "\n"
  fi
  if [ ! -z $OUTPUT ] # write conditional effect of TEST_HAPLOTYPE[0] (HR, p value) in supplied OUTPUT file
  then
    Rscript ${HOME_DIR/%/\/}ukbb_haplotype_model.v2.R -i ${INCLUDED_HAPLOTYPES} -t ${TEST_HAPLOTYPES} -p $PHENOTYPES -k $((i+1)) -g "$groups" -v 1 -n $((${#group_array[@]}-${#JOIN[@]})) > routput.${NAME/%/.}txt
