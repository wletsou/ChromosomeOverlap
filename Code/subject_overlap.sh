#! /bin/bash
SNP_FILE=$1 #Matrix of snp genotypes (rows) x subjects (columns)
INDEX=$2 #indexes to join to each column
TRANSPOSE=$3 #whether to transpose input file from subjects (rows) x genotypes (columns); set value to any non-null value

file_ID=${SNP_FILE%.*} #get identifiers from SNP_FILE before the file extension
echo fileID is ${file_ID}
printf "\n"
if [ ! -z $TRANSPOSE ];
then
  n_fields=$(awk 'END{print NF}' $SNP_FILE)
  for i in `seq 1 $n_fields`
  do
    cut -f$i $SNP_FILE | paste -s
  ...
  #cp --remove-destination $SNP_FILE subjects_${INDEX}.txt
  n_subjects=$(awk 'END{print NF}' $SNP_FILE)
  n_snps=$(awk 'END{print NR}' $SNP_FILE)
fi
# n_subjects=$(awk 'END{print NF}' subjects_${INDEX}.txt)
# n_snps=$(awk 'END{print NR}' subjects_${INDEX}.txt)
...
#https://stackoverflow.com/questions/277485/how-can-i-repeat-a-string-n-times-in-perl
eval str=($str)
for partition in "${str[@]}"
do
  echo $partition
done > sterling_${file_ID}_${INDEX}.txt

echo List of possible partitionings of $INDEX:
cat sterling_${file_ID}_${INDEX}.txt
printf "\n"
...
code=$(echo sterling_${file_ID}_${INDEX}.txt | awk '{printf "awk -F\",\" \x27 BEGIN{OFS=\",\"} { for (i=1;i<NF;i++) if (gensub(\"[|][0-9]+\",\"\",\"g\",\$(i+1))<=gensub(\"[0-9]+[|]\",\"\",\"g\",\$i)) \$(i+1)=\"\"} {print}\x27 %s | awk -F\"|\" \x27 BEGIN{OFS=\"|\"} { for (i=1;i<NF;i++) if (gensub(\",[0-9]+\",\"\",\"g\",\$(i+1))<=gensub(\",[0-9]+\",\"\",\"g\",\$i)) \$(i+1)=\"\"} {print}\x27 > sterling_'${file_ID}'_'${INDEX}'_temp.txt",$0}')
echo $code
printf "\n"
eval "$code"
#https://www.math.utah.edu/docs/info/gawk_13.html#SEC126
...
let val=$val
str=$(echo ${expr}==${val})
code1=$(echo $str | awk '{printf "awk -F\"[,|]\" \x27%s {print}\x27 sterling_'${file_ID}'_'${INDEX}'_temp.txt > sterling_'${file_ID}'_'${INDEX}'_temp1.txt",$0}')
echo $code1
eval "$code1"
printf "\n"
cat sterling_${file_ID}_${INDEX}_temp1.txt
printf "\n"
...
let val=$val
str=$(echo ${expr}==${val})
code2=$(echo $str | awk '{printf "awk -F\"[,|]\" \x27%s {print}\x27 sterling_'${file_ID}'_'${INDEX}'_temp1.txt > sterling_'${file_ID}'_'${INDEX}'_subsets.txt",$0}')
echo $code2
eval "$code2"
printf "\n"
cat sterling_${file_ID}_${INDEX}_subsets.txt
printf "\n"

n_combos=$(awk 'END{print NR}' sterling_${file_ID}_${INDEX}_subsets.txt)
...
#https://stackoverflow.com/questions/40642558/finding-max-and-minimum-values-from-a-bash-array
...
do
  #pattern=$(awk "NR==$row {print; exit}" sterling_${INDEX}_subsets.txt | perl -pne "s/[|]/+/g")
  #extract line and replace | with + for naming of temporary combo file
  #echo -e "\n"
  #cat sterling_${file_ID}_${INDEX}_subsets.txt
  code_str=$(echo "awk 'NR==$row{print \$0}' sterling_${file_ID}_${INDEX}_subsets.txt")
  echo $code_str
  printf "\n"
  pattern=$(eval "$code_str")
  pattern=$(echo $pattern | perl -pne "s/[|]/+/g")
  echo Row $row from sterling_${file_ID}_${INDEX}_subsets.txt is $pattern
  #awk "NR==$row {print; exit}" sterling_${INDEX}_subsets.txt > combo_${INDEX}_${pattern}_temp.txt #paritioned index with | or ,
  code_str=$(echo "awk 'NR==$row{print \$0}' sterling_${file_ID}_${INDEX}_subsets.txt > combo_${file_ID}_${INDEX}_${pattern}_temp.txt")
  echo $code_str
  eval "$code_str"
  printf "\n"
  ...
  COMBO=$(awk -F\"\t\" "BEGIN{
    ...
    print
  }" combo_${file_ID}_${INDEX}_${pattern}_temp.txt) #sort numerically within bar group https://www.gnu.org/software/gawk/manual/html_node/Array-Sorting-Functions.html#Array-Sorting-Functions
  ...
  COMBO=$(awk -F\"\t\" "BEGIN{
    ...
    print
  }" combo_${file_ID}_${INDEX}_${pattern}_temp.txt) #sort the bar groups based on the value of the first comma-separated entry https://www.linuxquestions.org/questions/programming-9/awk-asorti-produces-unexpected-result-4175600691/
  ...
  #cp --remove-destination subjects_$INDEX.txt subjects_${INDEX}_$COMBO.txt
  ...
  do
    ...
    then
      ...
      echo cp \--remove-destination $SNP_FILE subjects_${file_ID}_${INDEX_NEW}_${COMBO_file}_${partition_file}.txt
      cp --remove-destination $SNP_FILE subjects_${file_ID}_${INDEX_NEW}_${COMBO_file}_${partition_file}.txt
    fi
    printf "\n"
    ...
    echo Chromosome tuples \(col 1\) with sharing pattern\(s\) \(col 2\) among indicated SNPs \(col 3\):
  ...
  # if [ -f subjects_${INDEX_NEW}_${COMBO_file}_${partition_file}.txt ];
  # then
  #   rm subjects_${INDEX_NEW}_${COMBO_file}_${partition_file}.txt
  # fi
  ...
  then
    ...
    do
      ...
      code3=$(echo $str1 | awk '{printf "%s",$0}')
      ...
      # code4=$(echo "awk -F\"\t\" 'BEGIN{OFS=FS} { if (length(\$2) == $n_comma) print \$0 }' snps_unique_${file_ID}_${INDEX_NEW}_${COMBO_file}_${partition_file}.txt > snps_unique_temp_${file_ID}_${INDEX_NEW}_${COMBO_file}_${partition_file}.txt")
...
for file in sterling_${file_ID}_${INDEX}*
do
  if [ -f $file ];
  then
    echo rm $file
