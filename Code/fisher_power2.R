...
option_list = list(make_option(c("-x","--x"),type="integer",help="Counts in cases"), make_option(c("-n","--n"),type="integer",help="Total chromosomes in cases"), make_option(c("-y","--y"),type="integer",help="Counts in controls"), make_option(c("-m","--m"),type="integer",help="Total chromosomes in controls"), make_option(c("-o","--OR"),type="numeric",help="Odds ratio to detect"), make_option(c("-a","--alpha"),type="numeric",help="False positive level",default=0.05), make_option(c("-i","--indiv"),action="store_true",help="Compute individual-level frequncies"))
...
p1 <- x1/n1
p2 <- x2/n2 # observed controls frequency
...
p1 <- OR * p2 /(1 - p2 + OR * p2) # expected cases frequency
