...
touch ${2%.*}.$(eval "printf '%0${char_h}d-%0${char_h}d' ${RANGE[0]} ${RANGE[1]}").translated.txt && echo touch ${2%.*}.$(eval "printf '%0${char_h}d-%0${char_h}d' ${RANGE[0]} ${RANGE[1]}").translated.txt && printf "\n"
for ((i=${RANGE[0]};i<=${RANGE[1]};i++))
do
  echo Haplotype $i of $((STEP_SIZE-1)).
