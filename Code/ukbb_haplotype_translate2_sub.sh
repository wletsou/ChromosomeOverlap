...
echo bsub \-P SJLIFE \-J \"myJob[1-$n_jobs]\" \-oo ukbb_haplotype_translate2.%I.out \-eo ukbb_haplotype_translate2.%I.err \-R \"rusage[mem=256]\" \-q standard \"${HOME_DIR}/ukbb_haplotype_translate2.sh ${POPULATION} $HAPLOTYPES $DELTA.\\\$LSB_JOBINDEX $DIRECTORY $HOME_DIR\"
bsub -P SJLIFE -J "myJob[1-$n_jobs]" -oo ukbb_haplotype_translate2.%I.out -eo ukbb_haplotype_translate2.%I.err -R "rusage[mem=256]" -q standard "${HOME_DIR}/ukbb_haplotype_translate2.sh ${POPULATION} $HAPLOTYPES $DELTA.\$LSB_JOBINDEX $DIRECTORY $HOME_DIR"
printf "\n"
