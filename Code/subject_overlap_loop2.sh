#! /bin/bash
# loops through all SIGMA-tuples of chromosomes in the columns of SNP_FILE and look for SNPs that are the same across the group.  Output the unique patterns and their counts for each group type.  Relies on subject_overlap.sh to do overlaps and comparisons
SNP_FILE=$1 # haplotype_estimates_transpose file of snps x chromosomes
SIGMA=$2 # (One fewer than) the number of samples to overlap
OUTPUT=$3 # Base file name for output Pattern.${OUTPUT}_*.txt
RANGE=$4 # subset of the tuples to use, indexed as "from,to" starting from 0 and ending at (2n choose sigma + 1) - 1
DIRECTORY=$5 # Folder to store output Pattern file, e.g. ./snp_files; use $PWD if none supplied
...
file_ID=${SNP_FILE%.*} # name of haplotype_estimates transpose file minus file extension

n_samples=$(awk 'BEGIN{nf=0} {if (NF>nf) {nf=NF} } END{print nf}' $SNP_FILE) # columns (includes rsid)
...
num0=$(($n_samples-1)) # total number of haplotypes excluding rsid column in $SNP_FILE
if (($num0>=$SIGMA))
then
  let num=1;
  let den=1
...
do
  ...
  str=${str}$(echo " do (( i$j > $((${out[$((j-1))]}+1)) )) && ll=\$((i$j+1)) || ll=$((${out[$((i-1))]}+1)); for i$i in \`seq \$ll $n_samples\`;") # if previous index i$j is not at its starting value, beging counting the current index i$((j+1)) from i$j+1; else use the initial value given in the out array
  # https://stackoverflow.com/questions/169511/how-do-i-iterate-over-a-range-of-numbers-defined-by-variables-in-bash
done
str=${str}$(echo " do INDEX[\$k]=\$i1") #increment INDEX key k and assign in the value i1,i2,...,iSIGMA
...
do
  ...
  while (($error!=0))
  do
    echo Now running "sh ${HOME_DIR}/subject_overlap.sh $SNP_FILE ${INDEX[$i]} 2> subject_overlap_error_${file_ID}_${INDEX[$i]}.err"
    str=$(echo "sh ${HOME_DIR}/subject_overlap.sh $SNP_FILE ${INDEX[$i]} 2> subject_overlap_error_${file_ID}_${INDEX[$i]}.err") # saves stderr (2) to a file
    printf "\n"
    code=$(echo $str)
    start=$(date +%s.%N)
    eval "$code"
