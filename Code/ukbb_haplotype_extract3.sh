...
echo bcftools view  \-r $region_str \--samples-file $samples_file \--force-samples \--output-type v \--header-only $VCF_FILES \| awk \'{printf \"sjlife\\t\"\; for \(i=10\;i\<=NF\;i++\) {printf \"%s\\t%s%s\",\$i,\$i,\(i\<NF?\"\\t\":\"\\n\"\)} }\' \| tail \-1 \> $transpose_file
bcftools view -r $region_str --samples-file $samples_file --force-samples -m2 -M2 -v snps --output-type v --header-only $VCF_FILES | awk '{printf "sjlife\t"; for (i=10;i<=NF;i++) {printf "%s\t%s%s",$i,$i,(i<NF?"\t":"\n")} }' | tail -1 > $transpose_file
printf "\n"
...
echo bcftools view \-r $region_str \--samples-file $samples_file \--force-samples \--output-type v \--no-header \--output-file $new_vcf_file $VCF_FILES
bcftools view -r $region_str --samples-file $samples_file --force-samples -m2 -M2 -v snps --output-type v --no-header --output-file $new_vcf_file $VCF_FILES
printf "\n"

echo Compress and index vcf file:
echo bcftools view \-r $region_str \--samples-file $samples_file \--force-samples \--output-type z \--output-file $(echo ${POPULATION[*]%.indiv*} | sed 's/ /+/g').hg19_chr${CHR[0]}.${BP_RANGE[0]}-${BP_RANGE[1]}.hg38.vcf.gz $VCF_FILES
bcftools view -r $region_str --samples-file $samples_file --force-samples -m2 -M2 -v snps --output-type z --output-file $(echo ${POPULATION[*]%.indiv*} | sed 's/ /+/g').hg19_chr${CHR[0]}.${BP_RANGE[0]}-${BP_RANGE[1]}.hg38.vcf.gz $VCF_FILES
printf "\n"
