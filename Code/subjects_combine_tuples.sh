#! /bin/bash
#Combine files with the same overlap and total the number of unique patterns; use as input to subjects_overlap_iteration.sh
POPULATION=$1 # single population name whose tuples are to be combined
CHR=$2
BP_RANGE=$3
COMBINE_GROUPS=$4 # whether to merge files with the same number of bar groups, e.g., 2,3+j 2,j+3 and 2+3,j form a two-group pattern
NAME=$5 # Default iteration name, Iteration000
DIRECTORY=$6
HOME_DIR=$7
...
for file in Pattern_combined${NAME}.${POPULATION}.chr${CHR}.${BP_RANGE[0]}-${BP_RANGE[1]}* #careful only to delete Pattern_combined files on Iteration000
do
  if [ -f $file ]
  then
    test -f $file && found+=1 # counter whether any files have been removed
...
unset ref_pattern
unset pattern_array
echo Pattern.${POPULATION}.chr${CHR}.${BP_RANGE[0]}-${BP_RANGE[1]}*.txt
printf "\n"
for file in ${DIRECTORY}/Pattern.${POPULATION}.*chr${CHR}.${BP_RANGE[0]}-${BP_RANGE[1]}*.txt
do
  ...
  do
    ...
    then
      ...
      touch Pattern_combined${NAME}.${POPULATION}.chr${CHR}.${BP_RANGE[0]}-${BP_RANGE[1]}_${pattern%.txt}.tmp
    fi
  done
done
...
do
  ...
  pattern_array=($(echo ${pattern%.txt} | perl -pne 's/[+]/,/g' | awk 'END{ split($0,array,","); m=asort(array); for (i=1;i<=m;i++) { printf("%s ",array[i]) } }'))
  # pattern_array=($(echo ${pattern%.txt} | perl -pne 's/([A-Za-z0-9]+)[,+]*/$1 /g'))
  ...
  echo File ${file} becomes ${file%_*}_${pattern}.tmp. Add to Pattern_combined${NAME}.${POPULATION}.chr${CHR}.${BP_RANGE[0]}-${BP_RANGE[1]}_${pattern}.tmp  # remove shortest matching string from right (i.e. to right of *)
  test -f ${file} && echo cp ${file} ${file%_*}_${pattern}.tmp && printf "\n"
  test -f ${file} && cp ${file} ${file%_*}_${pattern}.tmp

  test -f ${file%_*}_${pattern}.tmp && echo cat ${file%_*}_${pattern}.tmp \>\> Pattern_combined${NAME}.${POPULATION}.chr${CHR}.${BP_RANGE[0]}-${BP_RANGE[1]}_${pattern}.tmp && printf "\n"
  test -f ${file%_*}_${pattern}.tmp && cat ${file%_*}_${pattern}.tmp >> Pattern_combined${NAME}.${POPULATION}.chr${CHR}.${BP_RANGE[0]}-${BP_RANGE[1]}_${pattern}.tmp
...
for file in Pattern_combined${NAME}.${POPULATION}.chr${CHR}.${BP_RANGE[0]}-${BP_RANGE[1]}_*.tmp
do
  PATTERN=${file##*_}
  PATTERN=${PATTERN%.tmp*}
  ...
  str=${str}$(echo ")]+=\$1 } END{ for (row in array) { print array[row],row } }' ${DIRECTORY}/Pattern_combined${NAME}.${POPULATION}.chr${CHR}.${BP_RANGE[0]}-${BP_RANGE[1]}_${PATTERN}.tmp > ${DIRECTORY}/Pattern_combined${NAME}.${POPULATION}.chr${CHR}.${BP_RANGE[0]}-${BP_RANGE[1]}_${PATTERN}.unsorted.txt ")
  echo $str
  eval "$str"
  printf "\n"

  echo sort -k2 Pattern_combined${NAME}.${POPULATION}.chr${CHR}.${BP_RANGE[0]}-${BP_RANGE[1]}_${PATTERN}.unsorted.txt \> Pattern_combined${NAME}.${POPULATION}.chr${CHR}.${BP_RANGE[0]}-${BP_RANGE[1]}_${PATTERN}.txt
  sort -k2 Pattern_combined${NAME}.${POPULATION}.chr${CHR}.${BP_RANGE[0]}-${BP_RANGE[1]}_${PATTERN}.unsorted.txt > Pattern_combined${NAME}.${POPULATION}.chr${CHR}.${BP_RANGE[0]}-${BP_RANGE[1]}_${PATTERN}.txt
  printf "\n"

  echo Output file Pattern_combined${NAME}.${POPULATION}.chr${CHR}.${BP_RANGE[0]}-${BP_RANGE[1]}_${PATTERN}.txt has $(awk '{total+=$1} END{print total}' Pattern_combined${NAME}.${POPULATION}.chr${CHR}.${BP_RANGE[0]}-${BP_RANGE[1]}_${PATTERN}.txt) entries in $(awk 'END{print NR}' Pattern_combined${NAME}.${POPULATION}.chr${CHR}.${BP_RANGE[0]}-${BP_RANGE[1]}_${PATTERN}.txt) lines after sorting.
  printf "\n"
done

#remove combined file
for file in Pattern_combined${NAME}.${POPULATION}.chr${CHR}.${BP_RANGE[0]}-${BP_RANGE[1]}*unsorted.txt ];
do
  if [ -f $file ]
  then
    test -f $file && found+=1
...
for file in Pattern_combined${NAME}.${POPULATION}.chr${CHR}.${BP_RANGE[0]}-${BP_RANGE[1]}*tmp ];
do
  if [ -f $file ]
  then
    test -f $file && found+=1
