...
echo awk \'\(NR\>=$ll \&\& NR\<=$ul\){print \$2}\' $PATTERNS
patterns=($(awk '(NR>='$ll' && NR<='$ul'){print $2}' $PATTERNS)) && printf "\n"
...
do
  ...
  echo awk \'BEGIN{OFS=\"\\t\"\; pattern=\"${patterns[$i]}\"\; m=split\(pattern,pattern_array,\",\"\)\; for \(i=1\;i\<=m\;i++\) {b=gensub\(\"0*\([1-9][0-9]*\)_\([0-9]*\)\",\"\\\\1\",\"g\",pattern_array[i]\)\; c=gensub\(\"0*\([1-9][0-9]*\)_\([0-9]*\)\",\"\\\\2\",\"g\",pattern_array[i]\)\; array[b+1]=c} } { n_chromosomes[\$1]+=1\; count=1\; for \(i in array\) {if \(\$i!=array[i]\) {count=0\; break} }\; total[\$1]+=count\; } END{a=total[1]\; b=n_chromosomes[1]\-a\; c=total[0]\; d=n_chromosomes[0]\-c\; print pattern,a,b,c,d}\' haplotype_estimates_combined.txt \>\> $filename
  awk 'BEGIN{OFS="\t"; pattern="'${patterns[$i]}'"; m=split(pattern,pattern_array,","); for (i=1;i<=m;i++) {b=gensub("0*([1-9][0-9]*)_([0-9]*)","\\1","g",pattern_array[i]); c=gensub("0*([1-9][0-9]*)_([0-9]*)","\\2","g",pattern_array[i]); array[b+1]=c} } { n_chromosomes[$1]+=1; count=1; for (i in array) {if ($i!=array[i]) {count=0; break} }; total[$1]+=count; } END{a=total[1]; b=n_chromosomes[1]-a; c=total[0]; d=n_chromosomes[0]-c; print pattern,a,b,c,d}' haplotype_estimates_combined.txt >> $filename
  finish=$(date +%s.%N)
  awk 'BEGIN{duration='$finish'-'$start';printf "Completed in %0.2f s.\n",duration}'
  printf "\n"
done
