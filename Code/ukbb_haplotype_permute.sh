...
do
  ...
  echo python ${HOME_DIR}/ukbb_row_sample.py \-f $file \-no_header \-n $n_samples \-s $SEED
  python ${HOME_DIR}/ukbb_row_sample.py -f $file -no_header -n $n_samples -s $SEED && printf "\n"
  # sample n_samples lines to be cases

  test -f ${file%.*}.row_sample_${n_samples}.txt && echo cat ${file%.*}.row_sample_${n_samples}.txt \>\> ${NAMES[0]}.indiv
  test -f ${file%.*}.row_sample_${n_samples}.txt && cat ${file%.*}.row_sample_${n_samples}.txt >> ${NAMES[0]}.indiv && printf "\n" # permuted cases
  ...
  echo awk \'NR==FNR{seen[\$1]\; next} \(\$1 in seen==0\){print \$0}\' $file cases.tmp \> cases.tmp1
  awk 'NR==FNR{seen[$1]; next} ($1 in seen==0){print $0}' $file cases.tmp > cases.tmp1 && printf "\n" # unpermuted cases who are not hetero- or homozygotes
  test -f cases.tmp1 && echo mv cases.tmp1 cases.tmp
  test -f cases.tmp1 && mv cases.tmp1 cases.tmp && printf "\n"

  echo awk \'NR==FNR{seen[\$1]\; next} \(\$1 in seen==0\){print \$0}\' $file controls.tmp \> controls.tmp1
  awk 'NR==FNR{seen[$1]; next} ($1 in seen==0){print $0}' $file controls.tmp > controls.tmp1 && printf "\n" # unpermuted controls who are not hetero- or homozygotes
  test -f controls.tmp1 && echo mv controls.tmp1 controls.tmp
  test -f controls.tmp1 && mv controls.tmp1 controls.tmp && printf "\n"

  test -f $file && echo rm $file
  test -f $file && rm $file && printf "\n"
  test -f ${file%.*}.row_sample_${n_samples}.txt && echo rm ${file%.*}.row_sample_${n_samples}.txt
  test -f ${file%.*}.row_sample_${n_samples}.txt && rm ${file%.*}.row_sample_${n_samples}.txt && printf "\n"
done
...
echo awk \'BEGIN{OFS=\"\\t\"} NR==FNR{seen[\$1]\; next} \(\$1 in seen\){\$3=0}1\' ${NAMES[1]}.indiv ${OUTPUT}.tmp \>\> ${OUTPUT}
awk 'BEGIN{OFS="\t"} NR==FNR{seen[$1]; next} ($1 in seen){$3=0}1' ${NAMES[1]}.indiv ${OUTPUT}.tmp >> ${OUTPUT} && printf "\n" # replace observed controls with permuted controls
