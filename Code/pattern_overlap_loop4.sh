...
then
  ...
  do
    ...
    # str=${str}$(echo "{ total[sprintf(\"%d\",(k % (${range[1]}-${range[0]}+1))+1)]=\$i1")
    str=${str}$(echo "{ total[sprintf(\"%d\",(k % (${range[1]}-${range[0]}+1))+1)]=1")
    for ((i=2;i<=${#out[@]};i++))
    do
      # str=${str}$(echo "*\$i$i")
      str=${str}$(echo "*1")
    done
    str=${str}$(echo "; k+=1; if (k>${range[1]}) {next} ")
    for ((i=${#out[@]};i>=1;i--))
    do
      str=${str}$(echo "} ")
    done
    ...
    str=${str}$(echo "if (i$((${#out[@]}-1))>${out[$((${#out[@]}-2))]}) {ll=i$((${#out[@]}-1))+1} else {ll=${out[$((${#out[@]}-1))]}}; for (i${#out[@]}=ll;i${#out[@]}<=$n_rows;i${#out[@]}++) { n${#out[@]}=split(\$i${#out[@]},snps_${#out[@]},\",\"); idx=0; printf \"%s\t\", total[sprintf(\"%d\",(k % (${range[1]}-${range[0]}+1))+1)]; for (j=1;j<=n${#out[@]};j++) { if (snps_${#out[@]}[j] in snps_1")
    for ((i=2;i<${#out[@]};i++))
    do
