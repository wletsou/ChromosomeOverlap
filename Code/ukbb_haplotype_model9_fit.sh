...
n_test_haplotypes=$(awk 'NR==1{print NF-1}' ${TEST_HAPLOTYPES[0]}) # number of new haplotypes to be tested
...
do
  ...
  echo cp $INCLUDED_HAPLOTYPES ${INCLUDED_HAPLOTYPES%.*}.${NAME}haplotype_$((i+1)).txt # initialize haplotype count file
  cp $INCLUDED_HAPLOTYPES ${INCLUDED_HAPLOTYPES%.*}.${NAME}haplotype_$((i+1)).txt && printf "\n" # initialize haplotype count file

  for ((j=0;j<${#TEST_HAPLOTYPES[@]};j++))
  do
    ...
    test_haplotype_name=$(awk 'NR==1{print $'$((i+2))'}' ${TEST_HAPLOTYPES[j]})
    ...
    echo awk \'NR==FNR{seen[\$1]=\$$((i+2))\; next} \(\$1 in seen\){print \$0,seen[\$1]}\' ${TEST_HAPLOTYPES[j]} ${INCLUDED_HAPLOTYPES%.*}.${NAME}haplotype_$((i+1)).txt \> ${INCLUDED_HAPLOTYPES%.*}.${NAME}haplotype_$((i+1)).tmp
    awk 'NR==FNR{seen[$1]=$'$((i+2))'; next} ($1 in seen){print $0,seen[$1]}' ${TEST_HAPLOTYPES[j]} ${INCLUDED_HAPLOTYPES%.*}.${NAME}haplotype_$((i+1)).txt > ${INCLUDED_HAPLOTYPES%.*}.${NAME}haplotype_$((i+1)).tmp && printf "\n" # concatenate column of test_haplotype i joined to included haplotype j

    test -f ${INCLUDED_HAPLOTYPES%.*}.${NAME}haplotype_$((i+1)).tmp && echo mv ${INCLUDED_HAPLOTYPES%.*}.${NAME}haplotype_$((i+1)).tmp ${INCLUDED_HAPLOTYPES%.*}.${NAME}haplotype_$((i+1)).txt
    test -f ${INCLUDED_HAPLOTYPES%.*}.${NAME}haplotype_$((i+1)).tmp && mv ${INCLUDED_HAPLOTYPES%.*}.${NAME}haplotype_$((i+1)).tmp ${INCLUDED_HAPLOTYPES%.*}.${NAME}haplotype_$((i+1)).txt && printf "\n"
  done

  # string of found haplotypes for each chromosome
  echo awk \'BEGIN{OFS=\"\\t\"} {str=\"\"\; for \(i=2\;i\<=NF\;i++\) {str=sprintf\(\"%s%s%s\",str,\(FNR==1 \&\& i\>2?\"+\":\"\"\),\$i\)}\; printf \"%s\\t%s\\n\",\$1,str }\' ${INCLUDED_HAPLOTYPES%.*}.${NAME}haplotype_$((i+1)).txt \> ${INCLUDED_HAPLOTYPES%.*}.${NAME}haplotype_$((i+1)).tmp
  awk 'BEGIN{OFS="\t"} {str=""; for (i=2;i<=NF;i++) {str=sprintf("%s%s%s",str,(FNR==1 && i>2?"+":""),$i)}; printf "%s\t%s\n",$1,str }' ${INCLUDED_HAPLOTYPES%.*}.${NAME}haplotype_$((i+1)).txt > ${INCLUDED_HAPLOTYPES%.*}.${NAME}haplotype_$((i+1)).tmp

  test -f ${INCLUDED_HAPLOTYPES%.*}.${NAME}haplotype_$((i+1)).tmp && echo mv ${INCLUDED_HAPLOTYPES%.*}.${NAME}haplotype_$((i+1)).tmp ${INCLUDED_HAPLOTYPES%.*}.${NAME}haplotype_$((i+1)).txt
  test -f ${INCLUDED_HAPLOTYPES%.*}.${NAME}haplotype_$((i+1)).tmp && mv ${INCLUDED_HAPLOTYPES%.*}.${NAME}haplotype_$((i+1)).tmp ${INCLUDED_HAPLOTYPES%.*}.${NAME}haplotype_$((i+1)).txt && printf "\n"

  # add haplotype counts for each chromosome, including enough columns for all observed haplotypes
  echo awk \'BEGIN{OFS=\"\\t\"} NR==FNR{ if \(NR\>1\) {seen[\$2]}\; next } \(NR!=FNR\){ if \(FNR==1\) {printf \"%s\\t\",\"sid\"\; m=asorti\(seen,haps\)\; for \(j=1\;j\<=m\;j++\) {printf \"%s%s\",haps[j],\(j\<m?\"\\t\":\"\\n\"\)\; col[haps[j]]=j+1} } else {b=gensub\(\"\([0-9]*\)_.*\",\"\\\\1\",\"g\",\$1\)\; chr[b]+=1\; array[sprintf\(\"%s\,%s\"\,b\,\$2\)]+=1\; if \(chr[b]==2\) {\$1=b\; for \(i=2\;i\<=m+1\;i++\) {\$i=0}\; for \(j in array\) {if \(j \~ b\) {c=gensub\(\".*[,]\([0-9]*\)\",\"\\\\1\",\"g\",j\)\; \$col[c]=array[j]\; delete array[j]} }\; print \$0 } } }\' ${INCLUDED_HAPLOTYPES%.*}.${NAME}haplotype_$((i+1)).txt ${INCLUDED_HAPLOTYPES%.*}.${NAME}haplotype_$((i+1)).txt \> ${INCLUDED_HAPLOTYPES%.*}.${NAME}haplotype_$((i+1)).tmp
  ...
  # echo awk \'BEGIN{OFS=\"\\t\"} NR==FNR{age[\$1]=\$4\; diag[\$1]=\$3\; next} \(\$1 in age\){if \(FNR==1\) {printf \"%s\\t%s\\t%s\\n\",\$0,\"age\",\"affected\"} if \(FNR \> 1\) {print \$0,age[\$1],diag[\$1]} }\' $PHENOTYPES ${INCLUDED_HAPLOTYPES%.*}.${NAME}haplotype_$((i+1)).txt \> ${INCLUDED_HAPLOTYPES%.*}.${NAME}haplotype_$((i+1)).tmp
  # awk 'BEGIN{OFS="\t"} NR==FNR{age[$1]=$4; diag[$1]=$3; next} ($1 in age){if (FNR==1) {printf "%s\t%s\t%s\n",$0,"age","affected"} if (FNR > 1) {print $0,age[$1],diag[$1]} }' $PHENOTYPES ${INCLUDED_HAPLOTYPES%.*}.${NAME}haplotype_$((i+1)).txt > ${INCLUDED_HAPLOTYPES%.*}.${NAME}haplotype_$((i+1)).tmp && printf "\n" # total haplotype counts for each individual; get age and affected status

  max_pcs=10 # maximum number of pcs to include in model
  char_pcs=$(($(echo $max_pcs | wc -c)-1)) # number of characters
  # get age and affected status and first 10 pcs
  echo awk \'BEGIN{OFS=\"\\t\"} NR==1{for \(i=1\;i\<=NF\;i++\) {if \(\$i~\"pc\"\) {b=gensub\(\"pc\([0-9]*\)\",\"\\\\1\",\"g\",\$i\)\; pc[b,\$1]=\$i\; col_pc[b]=i}\; if \(\$i==\"age.end\"\) {col_age=i\; age[\$1]=\$i}\; if \(\$i==\"diag\"\) {col_diag=i\; diag[\$1]=\$i} }\; next } \(NR==FNR \&\& NR\>1\){age[\$1]=\$col_age\; diag[\$1]=\$col_diag\; for \(i=1\;i\<=$max_pcs\;i++\) \{ if \(i in col_pc\) {pc[i\,\$1]=\$col_pc[i]\} \}\; next} \(\$1 in age\){if \(FNR==1\) {printf \"%s\",\$0\; for \(i=1\;i\<=$max_pcs\;i++\) {printf \"\\tpc%0${char_pcs}d\",i}\; printf \"\\t%s\\t%s\\n\",\"age\",\"affected\" } if \(FNR \> 1\) {printf \"%s\",\$0\; for \(i=1\;i\<=$max_pcs\;i++\) {printf \"\\t%0.5f\",pc[i,\$1]}\; printf \"\\t%s\\t%s\\n\",age[\$1],diag[\$1]} }\' $PHENOTYPES ${INCLUDED_HAPLOTYPES%.*}.${NAME}haplotype_$((i+1)).txt \> ${INCLUDED_HAPLOTYPES%.*}.${NAME}haplotype_$((i+1)).tmp
  awk 'BEGIN{OFS="\t"} NR==1{for (i=1;i<=NF;i++) {if ($i~"pc") {b=gensub("pc([0-9]*)","\\1","g",$i); pc[b,$1]=$i; col_pc[b]=i}; if ($i=="age.end") {col_age=i; age[$1]=$i}; if ($i=="diag") {col_diag=i; diag[$1]=$i} }; next } (NR==FNR && NR>1){age[$1]=$col_age; diag[$1]=$col_diag; for (i=1;i<='$max_pcs';i++) { if (i in col_pc) {pc[i,$1]=$col_pc[i]} }; next} ($1 in age){if (FNR==1) {printf "%s",$0; for (i=1;i<='$max_pcs';i++) {printf "\tpc%0'$char_pcs'd",i}; printf "\t%s\t%s\n","age","affected" } if (FNR > 1) {printf "%s",$0; for (i=1;i<='$max_pcs';i++) {printf "\t%0.5f",pc[i,$1]}; printf "\t%s\t%s\n",age[$1],diag[$1]} }' $PHENOTYPES ${INCLUDED_HAPLOTYPES%.*}.${NAME}haplotype_$((i+1)).txt > ${INCLUDED_HAPLOTYPES%.*}.${NAME}haplotype_$((i+1)).tmp && printf "\n" # total haplotype counts for each individual; get age and affected status

  test -f ${INCLUDED_HAPLOTYPES%.*}.${NAME}haplotype_$((i+1)).tmp && echo mv ${INCLUDED_HAPLOTYPES%.*}.${NAME}haplotype_$((i+1)).tmp ${INCLUDED_HAPLOTYPES%.*}.${NAME}haplotype_$((i+1)).txt
  test -f ${INCLUDED_HAPLOTYPES%.*}.${NAME}haplotype_$((i+1)).tmp && mv ${INCLUDED_HAPLOTYPES%.*}.${NAME}haplotype_$((i+1)).tmp ${INCLUDED_HAPLOTYPES%.*}.${NAME}haplotype_$((i+1)).txt && printf "\n"

  echo Joining haplotype $hap_name:
  echo Rscript ${HOME_DIR}/ukbb_haplotype_model5.R file=${DIRECTORY}/${INCLUDED_HAPLOTYPES%.*}.${NAME}haplotype_$((i+1)).txt groups="$groups" verbose=1 n=$((${#group_array[@]}-${#JOIN[@]})) n_covs=$max_pcs
  if [ -z $OUTPUT ]
  then
    Rscript ${HOME_DIR}/ukbb_haplotype_model5.R file=${DIRECTORY}/${INCLUDED_HAPLOTYPES%.*}.${NAME}haplotype_$((i+1)).txt groups="$groups" verbose=1 n=$((${#group_array[@]}-${#JOIN[@]})) n_covs=$max_pcs
  fi
  if [ ! -z $OUTPUT ] # write conditional effect of TEST_HAPLOTYPE[0] (HR, p value) in supplied OUTPUT file
  then
    Rscript ${HOME_DIR}/ukbb_haplotype_model5.R file=${DIRECTORY}/${INCLUDED_HAPLOTYPES%.*}.${NAME}haplotype_$((i+1)).txt groups="$groups" verbose=1 n=$((${#group_array[@]}-${#JOIN[@]})) n_covs=$max_pcs > routput.${NAME}txt
    test -f routput.${NAME}txt && cat routput.${NAME}txt # print R output file
    printf "\n"
    cat routput.${NAME}txt | grep LRT | awk 'BEGIN{OFS="\t"} {a=gensub(".*HR = ([NA0-9e.+-]*.*),.*","\\1","g",$0); b=gensub(".*LRT p value = ([NA0-9e.+-]*).*","\\1","g",$0); c=gensub(".*frequency = ([NA0-9e.+-]*.*)$","\\1","g",$0); print a,b,c}' | sed 's/[^NA0-9e+-.]/ /g' | sed 's/\s\+/ /g' | awk '{n=split($0,array,/\s+/); printf "'$hap_name'\t"; for (i=1;i<=n;i++) {printf "%s%s",array[i],(i<n?"\t":"\n")} }' >> ${OUTPUT}
    # echo $output | awk 'BEGIN{OFS="\t"}; {a=gensub(".*HR = ([NA0-9e.+-]*).*","\\1","g",$0); b=gensub(".*LRT p value = ([NA0-9e.+-]*).*","\\1","g",$0); printf "%s\t%s\t%s\n","'$hap_name'",a,b}' >> $OUTPUT
    test -f ${INCLUDED_HAPLOTYPES%.*}.${NAME}haplotype_$((i+1)).txt && echo rm ${INCLUDED_HAPLOTYPES%.*}.${NAME}haplotype_$((i+1)).txt
    test -f ${INCLUDED_HAPLOTYPES%.*}.${NAME}haplotype_$((i+1)).txt && rm ${INCLUDED_HAPLOTYPES%.*}.${NAME}haplotype_$((i+1)).txt && printf "\n"
